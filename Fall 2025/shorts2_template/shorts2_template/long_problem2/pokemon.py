...
def read_data(filename):
    ...
    for line in file_handle:
        ...
        if line and line[0] != '#':
            ...
            if type1 not in pokemon_database:
                pokemon_database[type1] = {}
            pokemon_database[type1][name] = stats
    file_handle.close()
    return pokemon_database

def calculate_averages(pokemon_database):
    """
    Calculates the average stats for each Pokemon type.
    Args:
        pokemon_database: A two-level dictionary of Pokemon data.
    Returns:
    ...
    for p_type, pokemons in pokemon_database.items():
...
def main():
    ...
    if pokemon_data:
        averages_by_type = calculate_averages(pokemon_data)
        max_average_stats = find_max_averages(averages_by_type)
